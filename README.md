...
1. If a movie is found show the preview as a `MovieCard` and the add button;
    - the API returns `MovieData` or `ResponseError` (see the `types`)
    - don't forget to normalize the received `MovieData`
...
- Replace `<your_account>` with your Github username in the [DEMO LINK](https://Oleksii-Mishchenko.github.io/react_movies-list-fetch-movies/) and add it to the PR description.
