...
1. If a movie is not found show an IsError message below the input;
    - hide it after changing the title;
1. If a movie is found show the preview as a `MovieCard` and the add button;
    - the API return `MovieData` or `ReposnseIsError` (see the `types`)
...
- Replace `<your_account>` with your Github username in the [DEMO LINK](https://serhii-yunnikov.github.io/react_movies-list-fetch-movies/) and add it to the PR description.
